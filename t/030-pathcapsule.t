...
BEGIN {
    ...
    use_ok( 'Build::Hopen::PathCapsule' ) || print "Bail out!\n";
}

my ($dut, $dut2, $vol, $path, $fn);

# Initialization {{{1
$dut = Build::Hopen::PathCapsule->new;
is($dut->abs, getcwd, 'Initializes to cwd');

$dut = Build::Hopen::PathCapsule->new(getcwd, 1);
is($dut->abs, getcwd, 'Manual initialization to cwd works');

$dut = Build::Hopen::PathCapsule->new('foo');
($vol, $path) = File::Spec->splitpath(getcwd, true);   # no fn
...
$dut = Build::Hopen::PathCapsule->new('/foo/bar', 1);
