...
$appveyor =~ m{^[A-Za-z0-9-]+/[A-Za-z0-9-]+}
  or die '--appveyor <GH username>/<GH repo>'
  if $appveyor;
$appveyor_badge //= $appveyor;
$appveyor_badge =~ m{^[A-Za-z0-9-]+/[A-Za-z0-9-]+}
  or die '--avbadge <GH username>/<GH repo>'
  if $appveyor_badge;

$travis =~ m{^[A-Za-z0-9-]+/[A-Za-z0-9-]+}
  or die '--travis <GH username>/<GH repo>'
  if $travis;
$travis_badge //= $travis;
$travis_badge =~ m{^[A-Za-z0-9-]+/[A-Za-z0-9-]+}
  or die '--trbadge <GH username>/<GH repo>'
  if $travis_badge;
...
while(my $line = <$fh>) {
    ...
    if($tweak_name && !$saw_name && $line =~ /NAME/) {
        ...
        $output .=
"[![Appveyor Status](https://img.shields.io/appveyor/ci/${appveyor_badge}.svg?logo=appveyor)](https://ci.appveyor.com/project/${appveyor}) "
          if $appveyor;
        $output .=
"[![Travis Status](https://img.shields.io/travis/${travis_badge}.svg?logo=travis)](https://travis-ci.org/${travis}) "
          if $travis;
