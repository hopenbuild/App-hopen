...
provide asset nodes to the generator and to later command nodes.

While the command graph is running:

=over

=item *

During the Check phase, the outputs are passed along.  Specifics of the
Check phase are TBD.

=item *

During the Gen phase, each command node, when run, outputs a C<made> arrayref
of L<App::hopen::Asset> instances.  These instances represent files or other
artifacts created by that command node.
