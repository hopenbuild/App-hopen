...
App::hopen::Manual - manual for the App::hopen build system

=head1 SYNOPSIS

L<App::hopen> is a cross-platform software build generator.  It makes files you
can pass to Make, Ninja, Visual Studio, or other build tools, to compile and
link your software.  This manual describes the operation and internals
of L<App::hopen>.

L<App::hopen> is built on L<Data::Hopen>, which is a flexible dataflow
processor and task runner.  L<App::hopen> uses dataflow graphs extensively.

Note: everything in this document is subject to change since L<App::hopen>
is currently under active development.

=head1 HOPEN'S PHILOSOPHY

=head2 "Show me"

Rather than telling the build system the pieces of your project,
show the build system how I<you> build your project.  Once you have a
successful workflow for building your project, you can translate it (more
or less) directly into hopen files.

=head2 Turing completeness

Building software is just as complicated as writing that software.  Therefore,
any build system not Turing complete will eventually hit its limits and need
to be monkey-patched, worked around, or supplemented.  Rather than dealing
with that, hopen uses all Turing-complete components from the get-go.
...
to touch files in that repo.  This is especially useful, e.g., for
L<Cygwin|https://cygwin.com/> and similar operating environments.
...
The default directory is C<< <project dir>/built >>, though, so you don't
have to manually create a build directory.
...
Toolsets are grouped under C<App::hopen::T>.
...
=item Language-support package ("LSP")

A subclass of L<App::hopen::Lang> that provides support for a particular
...
Hash keys starting with C<=> are also reserved.
...
=item L<App::hopen::HopenFileKit>

Gives you, e.g., L<App::hopen::HopenFileKit/on>.
...
The package name of the current C<< App::hopen::T::<stem> >> top-level class.

=item $Build

The L<Data::Hopen::G::DAG> instance representing the current build.
Goals in C<$Build> will become, e.g., top-level targets of a
generated C<Makefile>.  The function L<App::hopen::HopenFileKit/rule> is
...
command line).  An instance of an L<App::hopen::Phase> subclass.
...
After this phase, the user can set options by editing MY.hopen.pl.
See L</CUSTOMIZING A BUILD> for details.
A better interface for this is TBD.
...
This phase consists solely of running make(1), ninja(1), or
...
Hopen itself does not take any action during this phase, so it is different
from the other phases.

=head1 CUSTOMIZING A BUILD

After the L</Check> phase, you can edit the C<MY.hopen.pl> file to adjust what
will happen in the L</Gen> phase.  An example C<MY.hopen.pl> file might
look like:

    ...
    set_phase 'Gen';
    do {
        my ($Configuration, $__R_new_data);
        $Configuration = {
            'compiler @ App::hopen::T::Gnu' => [ '/usr/bin/x86_64-linux-gnu-gcc' ]
        };
...
You can edit the values under C<Configuration>, and those values will be
used during the L</Gen> phase.  See L</Configuration keys> for name formats.

=head1 WHAT HOPEN DOES

During each phase except Build:
...
provide outputs to the generator and to later command nodes.  The nature of
the outputs depends on the phase:
and L<App::hopen::Asset> instances during the Gen phase.

=over

=item *

During the Check phase, command nodes output configuration entries, and
forward any inputs they receive.

=item *

During the Gen phase, each command node outputs a (possibly empty) C<made>
arrayref of L<App::hopen::Asset> instances.  These instances represent files or
other artifacts created by that command node.

The generator receives information from each command node about work to
transform that node's C<made> inputs into outputs.

=back

In the Gen phase, after the command graph is traversed, the generator's
L<_finalize|App::hopen::Gen/_finalize> metnod is called to produce the
blueprint files.

=back

=head2 The execution environment (scope) of a command-graph node

Each command-graph node is (via inheritance) a L<Data::Hopen::G::Runnable>.
A command-graph node does its work in overridden function
L<Data::Hopen::G::Runnable/_run>.  Inside C<_run()>,
L<Data::Hopen::G::Runnable/scope> (available as C<< $self->scope >>) is a
key-value store of all the information
available to the node.

Keys starting with C<=> are reserved by hopen.  Treat them as read-only.
(C<=> because that character may not be used in an environment variable's name
under both Windows and POSIX.)

The information in a command-graph node's C<scope> includes the following.
In case of name conflicts, earlier items override later items.
...
=item C<=GeneratorClass>

The name of the current generator class

=item C<=ToolsetClass>

The name of the current toolset class

=item the outputs from the current hopen file(s)

All the members of any hash(es) returned by hopen file(s) in this run.   The
hopen file(s) are run before the command graph is traversed, because the
hopen file(s) create the command graph when run!  Therefore, data created by
the hopen file(s) is available to the command graph.

=item the environment

All the variables in the current process's environment

=back

=head2 Configuration keys

Configuration entries in a MY.hopen.pl file are represented by
L<App::hopen::Util::Thunk> instances.  Command nodes that have user-configurable
parameters must represent those parameters using a C<Thunk> having a
L<name|App::hopen::Util::Thunk/name> obeying one of the following patterns.
The maximum length of a name is 40 characters.  (TODO i18n names??)

=over

=item A namespaced name

A name of the form C<VAR[/VAR...] @ PACKAGE>.  For example,
C<compiler @ App::hopen::T::Gnu>, or C<make/flags @ App::hopen::G::Make>.
The C<VAR> is put first so that it will be easier for the user to read.

C<VAR> may contain any characters except slashes and at signs (C<@>),
but may not start or end with whitespace.
C<PACKAGE> must be the Perl package name of the component
(generator, toolset, or LSP) creating or using the namespaced name.
There must be exactly one space before and after the at sign.

If multiple commands use a namespaced name, they will all have the
same value.  TODO fix this in Thunk &c. --- C<< tgt=>undef >> means use the
default; first one wins; error if nothing defined a required variable;
warn (and first one wins) if two variables try to define the same name.

=item A local name

A name that does not match the form above.  It must not contain slashes or
at signs, for the avoidance of confusion.  If two commands use the same
local name, the names will be made unique so that they do not collide.

The name C<0> is prohibited.

=back

Command nodes output configuration entries as hash elements keyed by the
configuration key (whence the name!).
...
TODO?
    ...
    - In Manual, define the formats for languages and opcodes.
...
OLD - App::hopen::Phase - Namespace for hopen phases.
